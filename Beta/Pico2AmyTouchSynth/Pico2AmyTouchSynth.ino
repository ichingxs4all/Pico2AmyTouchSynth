...
// ── Knob assignment (KnobFunc index stored as float for MenuItem) ─────────────
float leftKnobAssign  = (float)KNOB_PITCHBEND;
float rightKnobAssign = (float)KNOB_MODULATION;

// Current normalised knob values (0-1) for the play-mode bar display
float leftKnobValue  = 0.5f;
float rightKnobValue = 0.5f;

// ── Knob function name tables (used by menu.ino and display.ino) ──────────────
// Full names for the menu picker; short 3-char labels for the play-mode bars.
static const char *kKnobFuncNames[] = {
    "Pitch Bend", "Modulation", "Chorus Lvl", "Echo Lvl", "Reverb Lvl"
};
static const char *kKnobFuncShort[] = {
    "PB", "MOD", "CHO", "ECH", "REV"
};
...
void setup() {
    ...
    setupButtonKnobs();      // seeds EMA; does NOT call AMY (not started yet)
    setupSettings();         // loads variables from EEPROM only — no AMY/I2C calls
    currentPatchIndex = bankSlotToIndex(currentBank, currentSlot);
    setupTouch();
    setupDisplay();
    setupAMY();              // starts AMY, loads patch at currentPatchIndex
