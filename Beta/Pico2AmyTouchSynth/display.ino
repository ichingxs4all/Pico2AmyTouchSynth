...
void updateDisplay() {
    ...
    if (patchSelectMode) {
        ...
        // Show label and bar for each knob's assigned function
        drawLabelledBar(kKnobFuncShort[(int)leftKnobAssign],   0, 47, 44, leftKnobValue);
        drawLabelledBar(kKnobFuncShort[(int)rightKnobAssign], 64, 47, 44, rightKnobValue);
    }

    // ── Row 56: patch number (left) + octave (right) ─────────────────────────
    // Octave = (lowest note) / 12.  PAD_NOTE[0]=48, transpose default=12 → oct 5.
    display.setCursor(0, 56);
    display.print("PATCH #");
    display.print(patchNum);

    int oct = (PAD_NOTE[0] + transpose) / 12;
    char octBuf[7];
    snprintf(octBuf, sizeof(octBuf), "OCT %d", oct);
    display.setCursor(OLED_WIDTH - (int)strlen(octBuf) * 6, 56);
